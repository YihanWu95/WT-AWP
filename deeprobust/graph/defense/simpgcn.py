...
from deeprobust.graph.defense import GraphConvolution, GraphConvolution2
...
class SimPGCN(nn.Module):
    ...
    def fit_mp(self, features, adj, labels, idx_train,proxy, idx_val=None, train_iters=200, initialize=True, verbose=False, normalize=True, patience=500, awp_weight = 1, awp_gamma = 1, awp_step = 1,awp_method = 'FSGM',idx_test = None,seed = 0,**kwargs):
...
#         print(self.gc1.delta.device)
        ...
        awp_adversary = AdvWeightPerturb(model=self, proxy=proxy, proxy_optim=proxy_opt, gamma=awp_gamma,step=awp_step,attack_method = awp_method,model_name = 'simpgcn')
        if initialize:
            self.initialize()
...
#             optimizer.step()
        ...
        for i in range(train_iters):
            ...
            output_nat,embeddings_nat = self.myforward(self.features, self.adj_norm)
            ...
            output_adv,embeddings_adv = self.myforward(self.features, self.adj_norm)
            loss_adv = F.nll_loss(output_adv[idx_train], labels[idx_train])
            loss = (1-awp_weight)*(loss_nat+self.lambda_ * self.regression_loss(embeddings_nat)) + awp_weight*(loss_adv) #+self.lambda_ * self.regression_loss(embeddings_adv))
