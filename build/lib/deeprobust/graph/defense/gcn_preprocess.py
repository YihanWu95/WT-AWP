...
class GCNSVD(GCN):
    ...
    def fit(self, features, adj, labels, idx_train, idx_val=None, k=50, train_iters=200, initialize=True, verbose=True, seed = 0,**kwargs):
        ...
        self.labels = labels
        super().fit(features, modified_adj, labels, idx_train, idx_val, train_iters=train_iters, initialize=initialize, verbose=verbose,seed = seed)
        
    def fit_mp(self, features, adj, labels, idx_train,proxy, idx_val=None, k=50, train_iters=200, initialize=True, verbose=False, normalize=True, patience=500, awp_weight = 1, awp_gamma = 1, awp_step = 5,awp_method = 'FSGM',idx_test = None,seed = 0,**kwargs):
        """First perform rank-k approximation of adjacency matrix via
        truncated SVD, and then train the gcn model on the processed graph,
        when idx_val is not None, pick the best model according to
        the validation loss.
        ...
        self.features = features
        self.labels = labels
        super().fit_mp( features, modified_adj, labels, idx_train,proxy, idx_val, train_iters=train_iters, initialize=initialize, verbose=verbose, normalize=normalize, patience=patience, awp_weight = awp_weight, awp_gamma = awp_gamma, awp_step = awp_step,awp_method = awp_method,idx_test = idx_test,seed = seed,**kwargs)
...
class GCNJaccard(GCN):
    ...
    def fit(self, features, adj, labels, idx_train, idx_val=None, threshold=0.01, train_iters=200, initialize=True, verbose=False, seed = 0,**kwargs):
    ...
    def fit_mp(self, features, adj, labels, idx_train,proxy, idx_val=None,threshold=0.01, train_iters=200, initialize=True, verbose=False, normalize=True, patience=500, awp_weight = 1, awp_gamma = 1, awp_step = 5,awp_method = 'FSGM',idx_test = None,seed = 0,**kwargs):
        """First drop dissimilar edges with similarity smaller than given
        threshold and then train the gcn model on the processed graph.
        When idx_val is not None, pick the best model according to the
        validation loss.
